...
from geodataset.utils.utils import CocoNameConvention, find_tiles_paths
...
class BaseLabeledRasterCocoDataset(BaseDataset, ABC):
    ...
    def __init__(
        self,
        fold: str,
        root_path: str | List[str] | Path | List[Path],
        transform: albumentations.Compose | None = None,
        other_attributes_names_to_pass: List[str] | None = None,
    ...
    def _reindex_coco_data(self, coco_data: dict):
        ...
        for category in coco_data["categories"]:
            ...
            if category_id not in self.category_id_to_metadata_mapping:
                ...
                and self.category_id_to_metadata_mapping[category_id]["name"] != category["name"]
            ):
                raise Exception(
    ...
    def _filter_unlabeled_excluded_class_tiles(self, exclude_classes: Optional[List[int]] = None):
        # Remove tiles without annotation for training
        original_tiles_number = len(self.tiles)
    ...
    def _remap_class_ids(self, skipped_classes):
        ...
        all_class_ids = sorted({label["category_id"] for tile in self.tiles.values() for label in tile["labels"]})
